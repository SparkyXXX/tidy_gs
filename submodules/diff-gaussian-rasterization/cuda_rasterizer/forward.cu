...
__device__ glm::vec3 computeCov2D(const glm::vec3 &mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, const float *cov3D, const float *viewmatrix)
{
							   ...
							   bool antialiasing)
{
	...
	const glm::vec3 p_orig = {orig_points[3 * idx], orig_points[3 * idx + 1], orig_points[3 * idx + 2]};
	glm::vec3 p_view = transformPoint4x3(p_orig, viewmatrix);
	if (p_view.z <= FRUSTUM_THERSOLD)
	{
		return;
	}
	...
	// Inverse 2D covariance and opacity neatly pack into one glm::vec4
	...
	float *__restrict__ invdepth)
{
	...
	glm::vec2 pixf = {(float)pix.x, (float)pix.y};
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (j % 32 == 0)
			{
				...
				{
					sampled_ar[(bbm * BLOCK_SIZE * NUM_CHANNELS_3DGS) + ch * BLOCK_SIZE + block.thread_rank()] = C[ch];
				}
			...
			glm::vec2 d = {xy.x - pixf.x, xy.y - pixf.y};
			glm::vec4 con_o = collected_conic_opacity[j];
